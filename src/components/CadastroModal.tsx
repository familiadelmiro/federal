  ...
  return (
    <Dialog.Root open={isOpen} onOpenChange={onClose}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-[50%] top-[50%] max-h-[90vh] w-[95vw] max-w-[900px] translate-x-[-50%] translate-y-[-50%] rounded-lg bg-primary-dark shadow-lg focus:outline-none z-50 overflow-y-auto">
          <VisuallyHidden.Root>
          ...
          <div className="relative">
            ...
            <div className="p-6 border-b border-white/20">
              <div className="text-center mb-6">
                ...
                <p className="text-gray-200 mb-6">Conheça como funciona nosso processo de associação</p>
              </div>
                ...
                {activationOptions.map((option, index) => (
                  ...
                  >
                    ...
                    <div className="flex items-center mb-4">
                      {option.icon}
                      <h4 className={`text-lg font-bold ml-3 ${option.highlight ? 'text-gray-900' : 'text-white'}`}>
                        {option.operator}
                      </h4>
                    </div>
                    
                    <h5 className={`text-base font-semibold mb-2 ${option.highlight ? 'text-gray-900' : 'text-white'}`}>
                      {option.title}
                    </h5>
                    
                    <p className={`text-sm ${option.highlight ? 'text-gray-600' : 'text-gray-200'}`}>
                      {option.description}
                    </p>
                  </div>
                ))}
            ...
            <div className="p-6">
              <div className="space-y-4">
                <button
                  ...
                  className="w-full bg-gradient-to-r from-accent to-accent-dark hover:from-accent-dark hover:to-accent text-white font-bold py-4 px-6 rounded-lg transition-all duration-300 transform hover:scale-105 shadow-lg flex items-center justify-center"
                >
